...
likelihoodMRA <- function(nu,phi,beta0,beta1,sigma2,taue,model,type,MRA_num){
  ...
  MRA.decompose <- function(j){
    ...
    for(i in 1:length(Wmat[[j]][[j]])){
      show(i)
      blocks_C[[i]] <- Wmat[[MRA_num]][[j]][[i]]
      blocks_Cstar[[i]] <- Wmat[[j]][[j]][[i]]
      blocks_Cstarinv[[i]] <- chol2inv(chol(blocks_Cstar[[i]]))
      blocks_Sigmaw[[i]] <- blocks_C[[i]] %*% blocks_Cstarinv[[i]] %*% 
        t(blocks_C[[i]])
      indices_blocks[[i]] <- rownames(Wmat[[MRA_num]][[j]][[i]])
      Sigmaw[rownames(Wmat[[MRA_num]][[j]][[i]]),
             rownames(Wmat[[MRA_num]][[j]][[i]])] <- blocks_Sigmaw[[i]]
      #      C[rownames(Wmat[[MRA_num]][[j]][[i]]),
      #      colnames(Wmat[[MRA_num]][[j]][[i]])] <- blocks_C[[i]]
   ...
   return(list(Sigmaw,blocks_C,blocks_Cstar,blocks_XSigmae,
