      ...
      for(l in 1:M){
        ...
        if(l!=1){
          ...
          for(k in 1:(l-1)){
            ...
            modify_depth(Wlist[[M]], 1,keep , .y >1) %>% modify_depth(1, f, Xlist[[M]][[jm]],Wlist[[M]][[l]][[jm]])
    ...
    if(M==nn){
      for(jm in 1:(dim(indicesW[[M]])[1])){
        ...
        for(l in 1:M){
          ...
          # f <- function(WL, A1, A3){
          #   t(A1%*%WL)%*%Sigma_m_inv%*%A1%*%A3
          # }
          # Atilde[[M]][[jm]][[l]] <- modify_depth(Wlist[[M]], 1, pluck,jm) %>% modify_depth(1, f, Xlist[[M]][[jm]],Wlist[[M]][[l]][[jm]])
